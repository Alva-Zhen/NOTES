...
1. `Promise.all(iterable)` 返回一个promise对象。
    ...
    iterable：是一个promise数组，数组中所有promise对象成功，则成功；若有一个失败，将失败信息作为返回结果。数组中也可以有普通值。
    ```
...
<p style="color:red">遗留问题就是finally方法的实现</p>
...
class Promise {
	...
	//返回一个设置了 finally 回调函数的Promise对象
		...
		return this.then(value => {
			//通过promise.resolve可以将onFinally转化为promise对象
			Promise.resolve(onFinally()).then(() => value)
		}, err => {
			Promise.resolve(onFinally()).then(() => err)
		})
	}
